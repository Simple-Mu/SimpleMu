...
namespace SimpleMu.ConnectServer;
...
public class ConnectServerDefinition : IConnectServerSettings
{
    ...
    /// <inheritdoc />
    IGameClientVersion IConnectServerSettings.Client =>
        Client ?? throw new InvalidOperationException("ConnectServerDefinition.Client not initialized.");

    /// <summary>
    ///     Gets or sets a value indicating whether the client should get disconnected when a unknown packet is getting
    ///     received.
    /// </summary>
    public bool DisconnectOnUnknownPacket { get; set; }

    /// <summary>
    ///     Gets or sets the maximum size of the packets which should be received from the client. If this size is exceeded,
    ...
    /// <inheritdoc />
    byte[] IConnectServerSettings.CurrentPatchVersion =>
        CurrentPatchVersion ?? throw new InvalidOperationException("Patch Version is not initialized");
